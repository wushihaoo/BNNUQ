...
from src.HMC.model_pbnn import *
from src.SGHMC.model_pbnn import *
...
# args = pbnnhmc_default_args
args = pbnnsghmc_default_args
...
if __name__ == '__main__':
    ...
    model_map = {
        'pbnnhmc': lambda: HMC_PBayesModel(dnnlayers=args['dnnlayers'],
                                        bnnlayers=args['bnnlayers'],
                                        prior_class=isotropic_gauss_prior(mu=0, sigma=args['prior_sigma']),
                                        dnnlr=args['dnnlr'],
                                        bnnlr=args['step_size'],
                                        n_steps=args['n_steps'],
                                        device=args['device']),
        'pbnnsghmc': lambda: SGHMC_PBayesModel(dnnlayers=args['dnnlayers'],
                                        bnnlayers=args['bnnlayers'],
                                        prior_class=isotropic_gauss_prior(mu=0, sigma=args['prior_sigma']),
                                        dnnlr=args['dnnlr'],
                                        bnnlr=args['lr'],
    ...
    t0 = time.time()
    model.train(train_dataset=train_dataset, map_epochs=args['map_epochs'], burn_in_epochs=args['burn_in_epochs'], sample_nets=args['sample_nets'], mix_epochs=args['mix_epochs'])
    t1 = time.time()

    train_time = t1 - t0
    print('训练时间：%.2f秒' % train_time)

    np.savez(os.path.join(result_dir, 'loss.npz'), map_loss=model.map_loss_log, loss=model.loss_log, err=model.err_log, prior=model.prior_log, llhd=model.llhd_log)
    plot_xy(x=np.arange(len(model.map_loss_log)), y=model.map_loss_log, title='map_loss', xlabel='epochs', ylabel='loss',
            save_path=os.path.join(result_dir, 'map_loss.png'), y_log=False)
