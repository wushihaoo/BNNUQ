...
# 用VI优化的贝叶斯神经网络
class VI_BayesNet(nn.Module):
    def __init__(self, layers, prior_class, init_out_noise=0.0):
        super(VI_BayesNet, self).__init__()
        
        self.layers = nn.ModuleList([VI_BayesLayer_LocalRepra(layers[i-1], layers[i], prior_class) for i in range(1, len(layers))])
...
# 用VI优化的贝叶斯神经网络模型
class VI_BayesModel(BaseNet):
    def __init__(self, layers, prior_class=isotropic_gauss_prior(mu=0, sigma=0.1), learning_rate=1e-3, device='cpu'):               # 网络结构，先验分布，学习率，gpu或cpu
        super(VI_BayesModel, self).__init__()
        self.layers = layers
        self.prior = prior_class
        self.lr = learning_rate
        self.device = device
        self.model_dir = './model'
        self.model_path = self.model_dir + '/vi_bayesmodel.pth'           # 模型保存路径

        self.net = VI_BayesNet(self.layers, self.prior)
