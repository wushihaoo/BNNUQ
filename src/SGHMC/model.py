...
# %%
# 用SGHMC优化的贝叶斯神经网络
class SGHMC_BayesNet(nn.Module):
    def __init__(self, layers, init_out_noise=0.0):
        super(SGHMC_BayesNet, self).__init__()
        ...
        # self.out_noise = torch.Tensor([0.0])
...
# 用SGHMC优化的贝叶斯神经网络模型
class SGHMC_BayesModel(BaseNet):
    def __init__(self, layers, prior_class=isotropic_gauss_prior(mu=0, sigma=0.1), lr=0.01, base_C=0.01, device='cpu'):               # 网络结构，先验分布，学习率，gpu或cpu
        super(SGHMC_BayesModel, self).__init__()
        self.layers = layers
        self.prior = prior_class
        self.lr = lr
        self.base_C = base_C
        self.device = device
        self.model_dir = './model/sghmc_net_samples'
        self.model_path = self.model_dir + '/sghmc_bayesmodel_0.pth'           # 模型保存路径
        self.net_id = 0
        self.epoch = 0
        
        self.net = SGHMC_BayesNet(self.layers)
        self.net.to(self.device)

        self.optimizer = SGHMC_Optimizer(self.net.parameters(), self.lr, self.base_C)
    ...
    # def llhd(self, y, output, output_noise):
    ...
    def train(self, train_dataset, burn_in_epochs, sample_nets, mix_epochs):
        ...
        for self.epoch in range(t_epochs):
            ...
            prior_epoch = 0
            llhd_epoch = 0
            burn_in = (self.epoch < burn_in_epochs)
            ...
            if (self.epoch >= burn_in_epochs) and (self.epoch % mix_epochs == 0):
                self.model_path = self.model_dir + '/sghmc_bayesmodel_%d' % self.net_id + '.pth'
