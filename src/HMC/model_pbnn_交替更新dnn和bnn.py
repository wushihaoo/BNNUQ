...
# %%
# 用HMC优化的贝叶斯神经网络
class HMC_PBayesNet(nn.Module):
    def __init__(self, dnnlayers, bnnlayers, init_out_noise=0.0):
        super(HMC_PBayesNet, self).__init__()
...
# 用HMC优化的贝叶斯神经网络模型
class HMC_PBayesModel(BaseNet):
    def __init__(self, dnnlayers, bnnlayers, prior_class=isotropic_gauss_prior(mu=0, sigma=0.1), dnnlr=0.01, bnnlr=0.01, n_steps=10, device='cpu'):               # 网络结构，先验分布，学习率，gpu或cpu
        super(HMC_PBayesModel, self).__init__()
        ...
        self.device = device
        self.model_dir = './model/hmc_pnet_samples'
        self.model_path = self.model_dir + '/pbnnhmc_bayesmodel_0.pth'           # 模型保存路径
        self.net_id = 0
        self.epoch = 0
        
        self.net = HMC_PBayesNet(self.dnnlayers, self.bnnlayers)
        self.net.to(self.device)

        self.dnnoptimizer = torch.optim.Adam(self.net.dnnlayers.parameters(), lr=self.dnnlr)
        self.optimizer = HMC_Optimizer(list(self.net.bnnlayers.parameters())+[self.net.out_noise], self.bnnlr, self.n_steps)
    ...
    # 用HMC算法采样模型参数
    def train(self, train_dataset, burn_in_epochs, sample_epochs):
        Nbatch = len(train_dataset)       # 训练集的batch数，小批量训练时计算复杂性代价comp需要除以batch数
        t_epochs = burn_in_epochs + sample_epochs
        switch_interval = 100
        ...
        for self.epoch in range(t_epochs):
            ...
            if (self.epoch < burn_in_epochs) and ((self.epoch // switch_interval) % 2 == 0):
                update_part = 'dnn'
            else:
                update_part = 'bnn'
            ...
            for x,y in train_loader:
                ...
                if update_part == 'dnn':
                    self.dnnoptimizer.zero_grad()
                    output = self.net(x)
                    loss = F.mse_loss(output, y)
                    loss.backward()
                    self.dnnoptimizer.step()
                else:
            ...
            if (self.epoch > switch_interval) and (self.epoch % 100 == 0):
                if self.epoch < burn_in_epochs:
                    print('Burn_in Epoch: %d, Posterior: %.4f, Error: %.4f' % (self.epoch, loss_epoch, error_epoch))
                else:
                    print('Sample Epoch: %d, Posterior: %.4f, Error: %.4f' % (self.epoch, loss_epoch, error_epoch))
                rates = self.optimizer.get_acceptance_rates()
                print('Acceptance Rate: ', [round(value, 4) for value in rates.values()])

            if self.epoch >= burn_in_epochs:
                self.model_path = self.model_dir + '/pbnnhmc_bayesmodel_%d' % self.net_id + '.pth'
