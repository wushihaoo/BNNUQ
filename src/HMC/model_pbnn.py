...
class HMC_PBayesModel(BaseNet):
    def __init__(self, dnnlayers, bnnlayers, prior_class=isotropic_gauss_prior(mu=0, sigma=0.1), dnnlr=0.01, bnnlr=0.01, n_steps=10, device='cpu'):               # 网络结构，先验分布，学习率，gpu或cpu
        ...
        self.optimizer = HMC_Optimizer(list(self.net.bnnlayers.parameters())+[self.net.out_noise], self.lr, self.n_steps)
    ...
    def map_loss(self, y, output):
        ...
        # 带l2正则项的均方误差
        map_loss = F.mse_loss(output, y) + sum(0.5*p**2 for p in self.net.parameters() if p.requires_grad)
        return map_loss
        ...
        for self.epoch in range(t_epochs):
            ...
            for x,y in train_loader:
                ...
                if train_mode == 'map':
                    ...
                    closure = make_closure(self.net, x, y, self.bnn_loss)
            ...
            if (self.epoch <= map_epochs) and (self.epoch % 100 == 0):
                print('Map Epoch: %d, Posterior: %.4f, Error: %.4f' % (self.epoch, map_loss_epoch, error_epoch))
